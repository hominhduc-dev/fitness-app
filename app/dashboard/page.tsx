...
export default async function DashboardPage() {
  ...
  const volumeUnitLabel = profile.preferredWeightUnit === "lbs" ? messages.dashboard.lbs : "kg"
  const dashboardSubtitle = workoutData.todayWorkout
    ? isVietnamese
      ? "Buổi tập hôm nay đã sẵn sàng, tập trung vào việc hoàn thành thật gọn và đúng nhịp."
      : "Today's workout is ready. Stay focused and move through it with clean momentum."
    : isVietnamese
      ? "Ngày nghỉ - phục hồi là chìa khoá để tiến bộ!"
      : "Rest day - recovery is key to progress!"
  const primaryAction = workoutData.todayWorkout
    ...
    : {
        description: isVietnamese ? "Mở danh sách workout để chuẩn bị buổi tập tiếp theo." : "Open workouts and line up the next session.",
        href: "/workout",
        label: messages.dashboard.quickWorkout,
      }
  const statCards = [
